...
class AmbassadorImageModel extends Model
{
    ...
    {	    	
    	...
    	{
    		$this->image_name = $this->createImageName();
    	
    		// save original image
    		$original = Yii::getAlias('@backend/uploads/img/ambassador/'. $this->image_name);
    		$this->image_file->saveAs($original);
    			
			// generate ratio thumbs
			$this->generateThumbs('ratio', $original, $this->image_name);
			
			// generate crop thumbs
			$this->generateThumbs('crop', $original, $this->image_name);
		}
    ...
    protected function createImageName()
    {
    	...
    	if (file_exists(Yii::getAlias('@backend/uploads/img/ambassador/'. $imgName)))
    	{
    ...
    {
    	$imgPath = Yii::getAlias('@ambassador/web/img');
    ...
    public function removeThumbs()
    {
		...
		foreach($types as $type)
		{
			...
			{
				$mode = $modes[$type];
				$file = $mode["path"] ."/". $format["width"] ."/". $this->image_name;
    	...
    	// delete original
    	$originalFile = Yii::getAlias('@backend/uploads/img/ambassador/'. $this->image_name);
